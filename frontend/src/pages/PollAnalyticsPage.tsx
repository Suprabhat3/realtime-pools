...
const genderLabel: Record<string, { icon: string; color: string; text: string }> = {
  MALE:             { icon: "♂", color: "text-blue-500",   text: "Male" },
  FEMALE:           { icon: "♀", color: "text-pink-500",   text: "Female" },
  NON_BINARY:       { icon: "⚧", color: "text-purple-500", text: "Non-binary" },
...
const PollAnalyticsPage = () => {
  ...
  const question = analytics.questions[0];
      ...
      <main className="flex-1 flex flex-col gap-8 max-w-4xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: "Options",            value: question?.optionCounts.length ?? 0, color: "text-gray-700" }
          ].map((stat) => (
        ...
        {question && (
          <>
            <div className="flex flex-col gap-1">
              <p className="text-xs font-bold tracking-widest uppercase text-gray-400">Poll Question</p>
